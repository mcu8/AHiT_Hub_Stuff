...
class mcu8_NoAbilitiesVolume extends Volume
	placeable;
...
simulated event Touch( Actor Other, PrimitiveComponent OtherComp, vector HitLocation, vector HitNormal )
{
    ...
    {
        Hat_PawnCombat(Other).GiveStatusEffect(class'mcu8_StatusEffect_NoAbilities');
    }
...
simulated event UnTouch( Actor Other)
{
    if (Enabled && Hat_PawnCombat(Other) != None)
    {
        Hat_PawnCombat(Other).RemoveStatusEffect(class'mcu8_StatusEffect_NoAbilities');
    }
    Super.UnTouch( Other );
}
